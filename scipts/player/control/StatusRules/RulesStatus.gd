...
func run_Status_debug() -> void:
	...
	if variables.debug_control_status:
		print(variables.slow_diminish)
...
func add_status(status_name: STATUSES, value := 0.5, duration := 3.0) -> void:
	if variables.debug_Control_Status: print(status_name,value,duration)
...
func get_slow_multiplier() -> float:
	if STATUSES.SLOW in character_status:
		...
		for i in range(stacks.size()):
			multiplier *= (1.0 - stacks[i]["value"] * variables.slow_diminish ** i) # diminishing return
		return multiplier
	return 1.0
